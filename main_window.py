...
class Ui(QMainWindow):
	def __init__(self):
		...
		uic.loadUi('ui/main.ui', self) # convert .ui file into python object
		...
		self.details_tab.serial_comboBox.currentIndexChanged.connect(self.serial_selected)
	...
	def detect_and_connect_605(self):
		...
		if len(ports) == 0:
			if QMessageBox.Cancel == QMessageBox.critical(self, "No devices found", "Need a least one device plugged in", QMessageBox.Ok | QMessageBox.Cancel):
				quit()
		...
		if self.shell.get_port_name() == None:
			if QMessageBox.Cancel == QMessageBox.critical(self, "Connection Failure", "Failed to find 605. May need to wait a couple of seconds if device was just powered on.", QMessageBox.Ok | QMessageBox.Cancel):
				quit()
	...
	def serial_selected(self, serial_num, from_db = False):
		if not from_db:
			if serial_num != "":
